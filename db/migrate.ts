...
async function migrateVersion() {
  ...
  const lastMigrationToBeExecuted = migrations.filter((it) => !!it.executedAt).slice(-1)[0];

  const migrationsToExecute = migrations.filter((it) => !it.executedAt);

  if (lastMigrationToBeExecuted) {
    console.log(`last migration to be executed: ${lastMigrationToBeExecuted?.name}`);
  }
...
async function migrateMake() {
  ...
  try {
    await fs.access(path.join(__dirname, "migrations"));
  } catch {
    await fs.mkdir(path.join(__dirname, "migrations"));
  }
